...
public enum NotesDirectoryRelocator {
    ...
    public static func relocate(
        ...
        if destinationDirectory.path().hasPrefix(sourceDirectory.path() + "/") {
        ...
        guard fileManager.fileExists(atPath: sourceDirectory.path(), isDirectory: &sourceIsDirectory),
              sourceIsDirectory.boolValue else {
            throw RelocationError(message: "The current notes folder could not be found.")
        }

        var destinationIsDirectory: ObjCBool = false
        if fileManager.fileExists(atPath: destinationDirectory.path(), isDirectory: &destinationIsDirectory) {
