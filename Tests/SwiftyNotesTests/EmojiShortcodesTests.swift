...
struct EmojiShortcodesTests {
    @Test("Known shortcodes render to their emoji")
    func knownShortcodesRender() {
    ...
    @Test("Plus and minus shortcodes render")
    func plusAndMinusShortcodesRender() {
    ...
    @Test("Unknown shortcode is left literal")
    func unknownShortcodeStaysLiteral() {
    ...
    @Test("Text with no colon is returned unchanged")
    func textWithoutColonIsUnchanged() {
        let input = "Just some plain prose without any shortcodes at all."
        #expect(EmojiShortcodes.render(input) == input)
    }

    @Test("Adjacent shortcodes both render")
    func adjacentShortcodesBothRender() {
    ...
    @Test("Empty and degenerate colon runs stay literal")
    func degenerateColonRunsStayLiteral() {
    ...
    @Test("Unterminated shortcode is left literal")
    func unterminatedShortcodeStaysLiteral() {
        #expect(EmojiShortcodes.render("look :smile at this") == "look :smile at this")
        #expect(EmojiShortcodes.render(":rocket") == ":rocket")
    }

    @Test("Colon inside non-shortcode text is untouched")
    func colonInPlainTextIsUntouched() {
    ...
    @Test("Shortcodes are case-sensitive lowercase like GitHub")
    func shortcodesAreCaseSensitive() {
        // gemoji keys are lowercase; uppercase must not match.
        #expect(EmojiShortcodes.render(":SMILE:") == ":SMILE:")
        #expect(EmojiShortcodes.render(":Rocket:") == ":Rocket:")
    }

    @Test("Overlong colon token does not match and stays literal")
    func overlongTokenStaysLiteral() {
        // Longer than the longest real shortcode — must bail to literal, not hang.
        let long = ":" + String(repeating: "a", count: 200) + ":"
        #expect(EmojiShortcodes.render(long) == long)
    }

    @Test("A failed match does not break a later valid one")
    func failedMatchDoesNotBreakLaterMatch() {
    ...
    @Test("An unknown well-formed span keeps its own colons (gemoji-greedy)")
    func unknownSpanKeepsItsColons() {
    ...
    @Test("Multiple matches across a longer string")
    func multipleMatchesAcrossLongerString() {
    ...
    @Test("The map is populated from the bundled gemoji table")
    func mapIsPopulatedFromBundledTable() {
